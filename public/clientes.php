        ...
        if ($nombre === '') {
            ...
            try {
                ...
                if ($idCliente > 0) {
                    ...
                    $stmt = $pdo->prepare(
                        'INSERT INTO clientes (nombre, direccion, telefono, id_vehiculo_marca, id_modelo, activo)
                         VALUES (:nombre, :direccion, :telefono, :id_vehiculo_marca, :id_modelo, :activo)'
                    );
                    $stmt->execute([
                        'nombre' => $nombre,
                        'direccion' => $direccion === '' ? null : $direccion,
...
if ($editId > 0) {
    try {
        $stmt = $pdo->prepare(
            'SELECT id_cliente, nombre, direccion, telefono, id_vehiculo_marca, id_modelo, activo
             FROM clientes
             WHERE id_cliente = :id_cliente
             LIMIT 1'
        );
